...
foreach ($environment in $envConfig) {
	...
	$basicAndEnvConfig = Merge-DataFile -Reference $basicConfig -Merge $environment.Config
	...
	if ($qaTestResults.Result -eq "Passed") {
		...
		$null = M365Configuration -ConfigurationData $mergedConfigData -OutputPath $outputPathDataFile
	}
	else {
